...
function Onboarding({ onComplete }) {
  ...
  // Clothing is collected as "spend per trip" — convert using frequency to a true monthly figure
  const clothingMonthly = (() => {
    const perTrip = parseFloat(clothingSpend) || 0;
    if (!clothingFreq || clothingFreq === "Rarely") return 0;
    if (clothingFreq === "Monthly") return perTrip;
    if (clothingFreq === "Every 2-3 months") return Math.round(perTrip / 2.5);
    if (clothingFreq === "Weekly") return Math.round(perTrip * 4.33);
    return perTrip;
  })();
  // Subscriptions: prefer the detailed per-service list (step 11) over the single rough estimate (step 8),
  // since the list is what actually powers the Bills tab and is the more accurate source of truth.
  const subscriptionsListTotal = subscriptionsList.reduce((a, s) => a + (parseFloat(s.amount) || 0), 0);
  const subscriptionsMonthly = subscriptionsList.length > 0 ? subscriptionsListTotal : (parseFloat(subscriptions) || 0);
  const totalFixed = (parseFloat(housingCost) || 0) + (parseFloat(carPayment) || 0) + (parseFloat(fuelCost) || 0) + (parseFloat(transitCost) || 0) + (parseFloat(healthInsurance) || 0) + (parseFloat(carInsurance) || 0) + (parseFloat(phoneBill) || 0) + (parseFloat(utilities) || 0) + subscriptionsMonthly;
  const totalVariable = ((parseFloat(groceries) || 0) * 4.33) + ((parseFloat(diningOut) || 0) * 4.33) + (parseFloat(entertainment) || 0) + (parseFloat(miscSpend) || 0) + clothingMonthly + lunchMonthly + vacationMonthly;
  ...
  const profileData = {
    ...
    clothingFreq, clothingSpend: parseFloat(clothingSpend) || 0, clothingMonthly,
...
// ── Modals ────────────────────────────────────────────────────────────────────
function WithdrawModal({ goal, onClose, onConfirm }) {
  const [step, setStep] = useState("warn");
  const [amount, setAmount] = useState("");
  if (!goal) return null;
  const amt = Math.min(parseFloat(amount) || 0, goal.saved);
  const handleFinish = () => {
    if (amt > 0) onConfirm(goal, amt);
    onClose();
    setStep("warn");
    setAmount("");
  };
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 100, display: "flex", alignItems: "flex-end", justifyContent: "center", backdropFilter: "blur(6px)" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: T.surface, borderRadius: "18px 18px 0 0", padding: 24, width: "100%", maxWidth: 420, border: `1px solid ${T.borderHi}`, borderBottom: "none" }}>
        ...
        {step === "amount" && (
          <>
            <h3 style={{ color: T.text, fontSize: 16, fontWeight: 700, marginBottom: 4 }}>Withdraw from {goal.name}</h3>
            <p style={{ color: T.textSub, fontSize: 12, marginBottom: 14 }}>Available: <span style={{ color: goal.color, fontWeight: 600 }}>${goal.saved.toLocaleString()}</span></p>
            <div style={{ position: "relative", marginBottom: 14 }}>
              <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: T.textSub }}>$</span>
              <input value={amount} onChange={e => setAmount(e.target.value)} type="number" placeholder="Amount ($)" max={goal.saved} style={{ ...S.input, paddingLeft: 28 }} />
            </div>
            <button onClick={() => setStep("confirm")} disabled={amt <= 0} style={{ ...S.primaryBtn(T.red), opacity: amt > 0 ? 1 : 0.5, cursor: amt > 0 ? "pointer" : "not-allowed" }}>Start {goal.cooldown}hr Cooldown</button>
          </>
        )}
        {step === "confirm" && (
          <>
            ...
            <p style={{ color: T.textMid, fontSize: 13, textAlign: "center", lineHeight: 1.6, marginBottom: 18 }}>Your {goal.cooldown}-hour timer has started for ${amt.toLocaleString()}. Use this time to reconsider — the withdrawal completes automatically once the timer ends.</p>
            <button onClick={handleFinish} style={S.primaryBtn()}>Understood</button>
          </>
        )}
      </div>
    </div>
...
function DepositModal({ goal, onClose, onConfirm }) {
  const [amount, setAmount] = useState("");
  ...
  const handleConfirm = () => {
    if (amt > 0) onConfirm(goal, amt, autoDeposit ? { freq: autoFreq, day: autoDay } : null);
    onClose();
  };
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 100, display: "flex", alignItems: "flex-end", justifyContent: "center", backdropFilter: "blur(6px)" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: T.surface, borderRadius: "18px 18px 0 0", padding: 24, width: "100%", maxWidth: 420, border: `1px solid ${T.borderHi}`, borderBottom: "none", maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <button onClick={handleConfirm} disabled={amt <= 0} style={{ ...S.primaryBtn(goal.color), opacity: amt > 0 ? 1 : 0.5, cursor: amt > 0 ? "pointer" : "not-allowed" }}>
...
function InvestTab({ initialHoldings = [], onPersistHolding = () => {}, onDeleteHolding = () => {} }) {
  ...
  const [portfolio, setPortfolio] = useState(() =>
    initialHoldings.map(h => ({
      id: h.id, amount: h.amount, vestingPeriod: h.vestingPeriod,
      inv: INVESTMENTS.find(i => i.ticker === h.ticker) || { ticker: h.ticker, name: h.ticker, color: T.textMid, icon: "dollarSign" },
    }))
  );
  ...
  const handleConfirm = (data) => {
    const holding = { ...data, id: Date.now() };
    setPortfolio(prev => [...prev, holding]);
    onPersistHolding({ id: holding.id, ticker: holding.inv.ticker, amount: holding.amount, vestingPeriod: holding.vestingPeriod });
    setVestInv(null);
    setConfirmed(data);
    ...
    <div style={{ padding: "0 20px", display: "flex", flexDirection: "column", gap: 14, paddingBottom: 20 }}>
      <div style={{ background: "rgba(245,166,35,0.1)", border: `1px solid ${T.gold}35`, borderRadius: 10, padding: "10px 12px", display: "flex", gap: 8, alignItems: "flex-start" }}>
        ...
        <p style={{ color: T.textMid, fontSize: 11, margin: 0, lineHeight: 1.6 }}>
          <strong style={{ color: T.gold }}>Practice mode.</strong> Prices shown are illustrative, not live market data, and no real money moves here. Use this to learn how allocating toward different assets works.
        </p>
      </div>
      {portfolio.length > 0 && (
        <div style={{ ...S.card }}>
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: T.surface, borderRadius: "24px 24px 0 0", width: "100%", maxWidth: 420, border: "1px solid rgba(255,255,255,0.09)", borderBottom: "none", maxHeight: "92vh", overflowY: "auto" }}>
        ...
        <div style={{ padding: "0 20px 32px", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div>
            ...
            <p style={{ color: T.textSub, fontSize: 11, margin: "6px 0 0" }}>We will remind you {reminder} day{reminder > 1 ? "s" : ""} before the {dueDay}{parseInt(dueDay) % 10 === 1 && parseInt(dueDay) % 100 !== 11 ? "st" : parseInt(dueDay) % 10 === 2 && parseInt(dueDay) % 100 !== 12 ? "nd" : parseInt(dueDay) % 10 === 3 && parseInt(dueDay) % 100 !== 13 ? "rd" : "th"} of each month.</p>
          </div>
...
function BillsTab({ profileSubs = [], initialBills = [], onPersist = () => {}, onDelete = () => {} }) {
  ...
  const getUrgency = (dueDay) => {
    ...
    return { label: `Due ${dueDay}${dueDay % 10 === 1 && dueDay % 100 !== 11 ? "st" : dueDay % 10 === 2 && dueDay % 100 !== 12 ? "nd" : dueDay % 10 === 3 && dueDay % 100 !== 13 ? "rd" : "th"}`, color: T.textSub, bg: "rgba(255,255,255,0.04)" };
  };
...
// ── Net Worth Tracker ─────────────────────────────────────────────────────────
function NetWorthTab({ goals, profile, initialAssets = [], initialLiabs = [], onPersistAsset = () => {}, onDeleteAsset = () => {}, onPersistLiab = () => {}, onDeleteLiab = () => {}, snapshots = [], onSnapshot = () => {} }) {
  ...
  // Record today's net worth once per day so a real trend can build over time
  useEffect(() => {
    onSnapshot(netWorth);
  ...
  const hasHistory = sortedSnaps.length >= 2;
  const history = hasHistory
    ? sortedSnaps.slice(-6).map(s => ({ mo: new Date(s.date + "T00:00:00").toLocaleDateString([], { month: "short", day: "numeric" }), nw: s.netWorth }))
    : [];
  const priorSnap = sortedSnaps.length >= 2 ? sortedSnaps[sortedSnaps.length - 2] : null;
  const change = priorSnap ? netWorth - priorSnap.netWorth : null;
  const minNW = hasHistory ? Math.min(...history.map(h => h.nw)) : 0;
  const maxNW = hasHistory ? Math.max(...history.map(h => h.nw)) : 0;
  const W = 340, H = 80;
  const pts = hasHistory ? history.map((h, i) => `${(i / (history.length - 1)) * W},${H - ((h.nw - minNW) / (maxNW - minNW || 1)) * (H - 12) - 6}`).join(" ") : "";
    ...
    <div style={{ padding: "0 20px", display: "flex", flexDirection: "column", gap: 14, paddingBottom: 24 }}>
      ...
      <div style={{ ...S.card, background: isPositive ? "linear-gradient(135deg, #0D1B30 0%, #0A2218 100%)" : "linear-gradient(135deg, #1A0D18 0%, #0D1B30 100%)", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 12 }}>
          {change !== null ? (
            <>
              <Icon name={change >= 0 ? "arrowUp" : "arrowDown"} size={13} color={change >= 0 ? T.green : T.red} />
              <span style={{ color: change >= 0 ? T.green : T.red, fontSize: 12, fontWeight: 600 }}>{change >= 0 ? "+" : "-"}${Math.abs(Math.round(change)).toLocaleString()} since last check-in</span>
            </>
          ) : (
            <span style={{ color: T.textSub, fontSize: 12 }}>We'll track your trend as you keep updating this</span>
          )}
        </div>
        ...
        {hasHistory ? (
          <>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
              {history.map((h, i) => <span key={i} style={{ color: T.textSub, fontSize: 10 }}>{h.mo}</span>)}
            </div>
          </>
        ) : (
          <div style={{ background: "rgba(255,255,255,0.03)", border: "1px dashed rgba(255,255,255,0.1)", borderRadius: 10, padding: "16px", textAlign: "center" }}>
            <p style={{ color: T.textSub, fontSize: 12, margin: 0, lineHeight: 1.6 }}>Your net worth trend will show up here once you've checked in on two different days. Come back tomorrow to start building your history.</p>
          </div>
        )}
      </div>
...
function CreditScoreTracker() {
  ...
  const [connected, setConnected]            = useState(false); // real bureau integration not yet built — do not default to true
...
const STATE_TAX_RATES = { AL:{name:"Alabama",rate:0.05}, AK:{name:"Alaska",rate:0}, AZ:{name:"Arizona",rate:0.025}, AR:{name:"Arkansas",rate:0.044}, CA:{name:"California",rate:0.093}, CO:{name:"Colorado",rate:0.044}, CT:{name:"Connecticut",rate:0.0699}, DE:{name:"Delaware",rate:0.066}, FL:{name:"Florida",rate:0}, GA:{name:"Georgia",rate:0.0539}, HI:{name:"Hawaii",rate:0.0825}, ID:{name:"Idaho",rate:0.058}, IL:{name:"Illinois",rate:0.0495}, IN:{name:"Indiana",rate:0.0305}, IA:{name:"Iowa",rate:0.038}, KS:{name:"Kansas",rate:0.057}, KY:{name:"Kentucky",rate:0.04}, LA:{name:"Louisiana",rate:0.03}, ME:{name:"Maine",rate:0.0715}, MD:{name:"Maryland",rate:0.0575}, MA:{name:"Massachusetts",rate:0.05}, MI:{name:"Michigan",rate:0.0425}, MN:{name:"Minnesota",rate:0.0785}, MS:{name:"Mississippi",rate:0.047}, MO:{name:"Missouri",rate:0.048}, MT:{name:"Montana",rate:0.059}, NE:{name:"Nebraska",rate:0.0584}, NV:{name:"Nevada",rate:0}, NH:{name:"New Hampshire",rate:0}, NJ:{name:"New Jersey",rate:0.0637}, NM:{name:"New Mexico",rate:0.049}, NY:{name:"New York",rate:0.0685}, NC:{name:"North Carolina",rate:0.045}, ND:{name:"North Dakota",rate:0.025}, OH:{name:"Ohio",rate:0.035}, OK:{name:"Oklahoma",rate:0.0475}, OR:{name:"Oregon",rate:0.0875}, PA:{name:"Pennsylvania",rate:0.0307}, RI:{name:"Rhode Island",rate:0.0599}, SC:{name:"South Carolina",rate:0.062}, SD:{name:"South Dakota",rate:0}, TN:{name:"Tennessee",rate:0}, TX:{name:"Texas",rate:0}, UT:{name:"Utah",rate:0.0455}, VT:{name:"Vermont",rate:0.066}, VA:{name:"Virginia",rate:0.0575}, WA:{name:"Washington",rate:0}, WV:{name:"West Virginia",rate:0.0482}, WI:{name:"Wisconsin",rate:0.053}, WY:{name:"Wyoming",rate:0}, DC:{name:"Washington D.C.",rate:0.085} };
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 14 }}>
        <label style={{ color: T.textSub, fontSize: 11, fontWeight: 600, display: "block", marginBottom: 5 }}>Pay frequency</label>
        <select value={payFreq} onChange={e => { logEvent("paycheck_freq_selected", { freq: e.target.value }); setPayFreq(e.target.value); }} style={{ width: "100%", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 8, padding: "9px 10px", color: T.text, fontFamily: "'Inter',sans-serif", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
          {Object.entries(PAY_FREQUENCIES).map(([key, f]) => (
      ...
      <Row label={`Base pay (${PAY_FREQUENCIES[payFreq]?.label})`} val={basePay} note={`${baseHrs} hrs a week at $${w}`} />
      {otHrs > 0 && <Row label={`Overtime pay (${PAY_FREQUENCIES[payFreq]?.label})`} val={otPay} color={T.gold} note={`${otHrs} hrs a week at $${(w * 1.5).toFixed(2)}`} />}
      <Row label={`Gross pay (${PAY_FREQUENCIES[payFreq]?.label})`} val={gross} color={T.text} />
...
export default function App() {
  ...
  const loadUserData = async (uid) => {
    ...
    try {
      ...
      if (prof && prof.monthly_income) {
        ...
        const fullSnapshot = (prof.full_data && typeof prof.full_data === "object") ? prof.full_data : {};
        setProfile({
          ...fullSnapshot,
  ...
  dbRows("bills", uid), dbRows("assets", uid), dbRows("liabilities", uid), dbRows("debts", uid),
      ...
      // Load net worth snapshots (last 90 days)
      const nwRows = await dbRows("networth_snapshots", uid, "&order=snapshot_date.desc&limit=90");
      if (Array.isArray(nwRows)) setDbSnapshots(nwRows.map(r => ({ date: r.snapshot_date, netWorth: Number(r.net_worth) })));

      // Load practice-invest holdings
  ...
  const saveProfile = async (p, uid) => {
    ...
    await fetch(`${SUPABASE_URL}/rest/v1/profiles`, {
      ...
      body: JSON.stringify({
        ...
        // Full snapshot of every onboarding field (age, dependents, vehicle, all budget
        // line items, subscriptions list, etc.) so nothing is lost on logout/login.
        // The scalar columns above are kept too, for simple SQL querying if ever needed.
        full_data: p,
...
const removeDebtDb  = (id) => { if (authUser) dbDelete("debts", id, authUser.id); setDbDebts(p => p.filter(x => x.id !== id)); };
  ...
  // ── Persist practice-invest holdings ───────────────────────────────
  ...
  const persistSnapshot = (netWorth) => {
    ...
    if (lastSnapshotRef.current.date === today && lastSnapshotRef.current.value === netWorth) return;
    lastSnapshotRef.current = { date: today, value: netWorth };
    dbUpsert("networth_snapshots", { id: `${authUser.id}_${today}`, user_id: authUser.id, snapshot_date: today, net_worth: netWorth });
    setDbSnapshots(prev => {
      const rest = prev.filter(s => s.date !== today);
      return [...rest, { date: today, netWorth }];
    });
  };
  ...
  // ── Actually apply a deposit/withdrawal to a goal's real saved amount ──
  const handleGoalDeposit = async (goal, amount, autoConfig) => {
    const updated = {
      ...goal,
      saved: goal.saved + amount,
      ...(autoConfig ? { autoDepositAmount: amount, autoDepositFreq: autoConfig.freq, autoDepositDay: autoConfig.day } : {}),
    };
    setGoals(prev => prev.map(g => g.id === goal.id ? updated : g));
    await saveGoal(updated);
  };
  const handleGoalWithdraw = async (goal, amount) => {
  ...
  const TABS = [
    ...
    { id: "credit",    icon: "shield",     label: "Credit", hidden: true    },
    ...
    { id: "invest",    icon: "trendUp",    label: "Invest (Practice)" },
...
if (screen === "newGoal") return <>{fonts}<GoalCreationFlow onComplete={g => { if (g) { setGoals(p => [...p, g]); saveGoal(g); } setScreen("app"); setTab("goals"); }} onCancel={() => setScreen("app")} /></>;
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, fontFamily: "'Inter',sans-serif", color: T.text, maxWidth: 420, margin: "0 auto", position: "relative", paddingBottom: 80, overflow: "hidden" }}>
      ...
      <WithdrawModal goal={withdrawGoal} onClose={() => setWithdrawGoal(null)} onConfirm={handleGoalWithdraw} />
